...
const BuyerDashboard = () => {
  ...
  const [stats, setStats] = useState({ totalSpent: 0, totalOrders: 0, monthly: [] });
  ...
  useEffect(() => {
    if (!buyerId) return;
    Promise.all([
      apiClient.get(`/reporting/buyer/${buyerId}`),
      apiClient.get(`/users/${buyerId}`)
    ]).then(([r1, r2]) => {
      setStats(r1.data || { totalSpent: 0, totalOrders: 0, monthly: [] });
      setProfile(r2.data?.user || profile);
    }).catch(() => {}).finally(() => setLoading(false));
  }, [buyerId]);

  const saveProfile = () => {
    apiClient.patch(`/users/${buyerId}`, profile).then((res) => setProfile(res.data.user)).catch(() => {});
  };

  return (
    <div className="min-h-screen bg-gray-50 text-black p-6 relative">
      {loading && <div className="absolute inset-0 z-10 bg-white/70"><Loading /></div>}
      <h1 className="text-2xl font-bold mb-6">Karshak</h1>
      <h2 className="text-xl font-semibold mb-6">Buyer Dashboard</h2>

      <div className="grid gap-6 w-full max-w-4xl md:grid-cols-2">
        <div className="bg-white shadow-md rounded-2xl p-6 border">
          <p className="text-gray-500 text-sm">Total Spent</p>
          <h3 className="text-3xl font-bold mt-2">₹ {Number(stats.totalSpent || 0).toFixed(2)}</h3>
        </div>
        <div className="bg-white shadow-md rounded-2xl p-6 border">
      ...
      <div className="w-full max-w-4xl mt-6 bg-white border rounded-2xl p-6">
        <p className="text-sm text-gray-500 mb-3">Monthly Spend</p>
